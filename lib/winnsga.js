...
function winnsga(winBackbone, globalConfig, localConfig)
{
    ...
    self.startEvolution = function(evoProps, seeds, evoError, finished)
    {
        ...
        if(self.evolutionRunning)
            throw new Error("Starting Evolution when it is already in progress and running!");
        ...
        if(self.evolutionInProgress)
        {
            ...
            asyncCall(internalGenerationLoop);
            finished();
        }
        else
        {
    ...
    function internalGenerationLoop()
    {
        //kill the loop -- we're no longer in progress
        ...
        self.evoObject.performOneGeneration()
            .then(function()
            {
                ...
                return qBackboneResponse("evolution:shouldEndEvolution", self.evoObject.generation, self.evoObject.activePopulation, self.evoObject.activeEvaluations);
            })
