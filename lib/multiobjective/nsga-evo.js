...
function evoContainer(evoProps, localConfig, backEmit, log)
{
	...
	self.multiobjective = new ranking(self.popEvaluations, qBackboneResponse, log);

  	function qBackboneResponse()
    {
        var defer = Q.defer();
        ...
        self.backEmit.apply(self, augmentArgs);
    ...
    {
    	toEval.fitness = Math.max(fromEval.fitness || 0, self.defaultFitness);
    	toEval.realFitness = Math.max(fromEval.realFitness || 0, self.defaultFitness);
    	toEval.behaviors = fromEval.behaviors || [];
    	toEval.complexity = fromEval.complexity || 0;
    }
     ...
     self.createOffSpring = function()
    {
        ...
        qBackboneResponse("generator:createArtifacts", self.genomeType, parentIxs.length, self.population, self.sessionObject)
  	...
  	self.evaluatePopulation= function()
    {
        ...
        qBackboneResponse("evaluate:evaluateArtifacts", self.population)
	...
	self.createInitialPopulation = function(evoProps, seeds)
	{
        ...
        qBackboneResponse("generator:createArtifacts", self.genomeType, popSize, seeds)
            .then(function(offspringObject)
            {
                ...
                //now we have our pop object all setup
                defer.resolve();
            })
            .fail(function(err)
            {
